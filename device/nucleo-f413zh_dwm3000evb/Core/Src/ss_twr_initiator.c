 ...
 * @author Decawave
 *
 * Edited by Johannes Arenander 2023.
 */
...
/* Length (bytes) of distance logging message. */
...
int ss_twr_initiator(void)
{
    ...
    if (dwt_configure(&config) == DWT_ERROR) return DWT_ERROR;
    ...
    while (1)
    {
         ...
         * Johannes Arenander
         */
	timeSeconds = elapsedTimeMilliseconds * 0.001;
	/* EDIT END */
        ...
        if (
            dwt_starttx(DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED) == DWT_ERROR
	) {
            while (1)
	    {
        	;
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                     ...
                     * Johannes Arenander
                     */
                    snprintf(message, 128, "%.3f,%.3f", timeSeconds, distance);
                    userLog(message);
                    /*
                    snprintf(message, 128, "%.3f,%.3f"USER_NEWLINE, distance, timeSeconds);
                    length = 1;
                    for (char *p = message; *p; p++) length++;
                    HAL_UART_Transmit(phCurrentUART, (uint8_t *)message, length, 1);
                    */
                    /* EDIT END */
                }
            }
